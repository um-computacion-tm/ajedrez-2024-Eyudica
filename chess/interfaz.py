...
class Juego:
        ...
        while coordenadas is None:
            try:
                ...
                if len(coordenadas) != 2:
                    raise ValueError("Tenes que ingresar exactamente 2 coordenadas.")
    ...
    def procesarJuego(self):
        if self.__juego_finalizado__:
            return 
        posicion_inicial, posicion_final = self.obtenerCoordenadas()
        if self.moverPieza(posicion_inicial, posicion_final):
         
            return True
        return False
    def mostrarGanador(self):
        if self.determinarGanador():
